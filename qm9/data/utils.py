...
def initialize_datasets(args, datadir, dataset, subset=None, splits=None,
                        ...
                        remove_h=False, finetune=False, basic_prob=False, bond_pred=False):
    """
    Initialize datasets.
    ...
    datasets = {split: ProcessedDataset(data, num_pts=num_pts.get(
        split, -1), included_species=all_species, subtract_thermo=subtract_thermo, basic_prob=basic_prob, bond_pred=bond_pred) for split, data in datasets.items()}
