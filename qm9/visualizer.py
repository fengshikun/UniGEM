...
def save_xyz_file(path, one_hot, charges, positions, dataset_info, id_from=0, name='molecule', node_mask=None, bfn_schedule=False, bond_info=None):
    try:
        os.makedirs(path)
    except OSError:
        pass
    ...
    frame_num = one_hot.size(0) if isinstance(one_hot, torch.Tensor) else len(one_hot)
    
    for batch_i in range(frame_num):
        f = open(path + name + '_' + "%03d.txt" % (batch_i + id_from), "w")
        if not bfn_schedule:
            f.write("%d\n\n" % atomsxmol[batch_i])
        else:
            f.write("%d\n\n" % len(positions[batch_i]))
        atoms = torch.argmax(one_hot[batch_i], dim=1)
        if not bfn_schedule:
            n_atoms = int(atomsxmol[batch_i])
        else:
            n_atoms = len(positions[batch_i])
        ...
        if bond_info is not None:
            bonds_edges, bond_types = bond_info[batch_i]
            for idx, bond in enumerate(bonds_edges):
                f.write("BOND %d %d %d\n" % (bond[0], bond[1], bond_types[idx]))
