...
def check_stability_bond(positions, atom_type, dataset_info, debug=False, bond_info=None):
    ...
    bond_indexes, bond_types = bond_info
    for idx, bond in enumerate(bond_indexes):
        ...
        if bond_type == 4:
            print('Bond type 4')
            bond_type = 1.5
        nr_bonds[i] += bond_type
        nr_bonds[j] += bond_type
...
def analyze_stability_for_molecules(molecule_list, dataset_info, bfn_schedule=False, bond_lst=None):
    ...
    #TODO forbidden bond pred
    # bond_lst = None
    
    for idx, mol in enumerate(processed_list):
        pos, atom_type = mol
        if bond_lst is not None:
            validity_results = check_stability_bond(pos, atom_type, dataset_info, bond_info=bond_lst[idx])
        else:
